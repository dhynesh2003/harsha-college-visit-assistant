...
function renderColleges(){const q=$("collegeSearch").value.toLowerCase();const rows=db.colleges.filter(c=>[c.name,c.location,c.contactName].join(" ").toLowerCase().includes(q));$("collegeGrid").innerHTML=rows.length?rows.map(c=>`<article class="entity-card"><div class="entity-top"><div><h3>${esc(c.name)}</h3><div class="meta">${esc(c.location||"Location not added")}</div></div><span class="pill">${esc(c.interest)}</span></div><div class="contact-box"><div class="title">${esc(c.contactName||"No main contact")}</div><div class="meta">${esc(c.contactDesignation||"Designation not added")}</div><div class="meta">${esc(c.contactPhone||"Phone not added")}</div></div><div class="next-box ${!c.nextAction?"warn":""}"><div class="title">Next: ${esc(c.nextAction||"No next action set")}</div><div class="meta">${fmt(c.nextDate)}</div></div><div class="button-row" style="margin-top:10px"><button class="btn ghost editCollege" data-id="${c.id}">Edit</button><button class="btn soft timelineCollege" data-id="${c.id}">Timeline</button></div></article>`).join(""):'<div class="empty">No colleges added yet.</div>';document.querySelectorAll(".editCollege").forEach(b=>b.onclick=()=>openForm("college",b.dataset.id));document.querySelectorAll(".timelineCollege").forEach(b=>b.onclick=()=>window.HARSHA_CRM_PRO?.openCollegeTimeline?.(b.dataset.id))}
