...
function showApp(){$("authView").classList.add("hidden");$("appView").classList.remove("hidden");$("userEmail").textContent=user?.email||"";renderAll();setTimeout(refreshPushStatus,200)}
...
function openForm(type,id="",date=""){
  ...
  if(type==="college"){$("collegeForm").reset();$("collegeDialog").showModal()}
...
$("collegeForm").onsubmit=async e=>{
  ...
  const payload={user_id:user.id,name:$("collegeName").value.trim(),location:$("collegeLocation").value.trim(),relevant_students:Number($("collegeStudents").value||0),stage:$("collegeStage").value,interest:$("collegeInterest").value,main_contact_name:$("collegeContactName").value.trim(),main_contact_designation:$("collegeContactDesignation").value.trim(),main_contact_phone:$("collegeContactPhone").value.trim(),main_contact_email:$("collegeContactEmail").value.trim(),next_action:$("collegeNextAction").value.trim(),next_action_at:$("collegeNextDate").value?new Date($("collegeNextDate").value).toISOString():null,blocker:$("collegeBlocker").value.trim(),notes:$("collegeNotes").value.trim(),updated_at:new Date().toISOString()};
  const {data,error}=await sb.from("colleges").insert(payload).select().single();if(error){setSync("error","Save failed");return alert(error.message)}
  if($("collegeContactName").value.trim()){await sb.from("contacts").insert({user_id:user.id,college_id:data.id,name:$("collegeContactName").value.trim(),designation:$("collegeContactDesignation").value.trim(),phone:$("collegeContactPhone").value.trim(),email:$("collegeContactEmail").value.trim(),preferred_contact:"Phone",note:"Main contact"})}
  closeDialog("collegeDialog");await loadAll();toast("College saved");
};
...
function renderColleges(){const q=$("collegeSearch").value.toLowerCase();const rows=db.colleges.filter(c=>[c.name,c.location,c.contactName].join(" ").toLowerCase().includes(q));$("collegeGrid").innerHTML=rows.length?rows.map(c=>`<article class="entity-card"><div class="entity-top"><div><h3>${esc(c.name)}</h3><div class="meta">${esc(c.location||"Location not added")}</div></div><span class="pill">${esc(c.interest)}</span></div><div class="contact-box"><div class="title">${esc(c.contactName||"No main contact")}</div><div class="meta">${esc(c.contactDesignation||"Designation not added")}</div><div class="meta">${esc(c.contactPhone||"Phone not added")}</div></div><div class="next-box ${!c.nextAction?"warn":""}"><div class="title">Next: ${esc(c.nextAction||"No next action set")}</div><div class="meta">${fmt(c.nextDate)}</div></div></article>`).join(""):'<div class="empty">No colleges added yet.</div>'}
function renderContacts(){$("contactGrid").innerHTML=db.contacts.length?db.contacts.map(c=>`<article class="entity-card"><div class="entity-top"><div><h3>${esc(c.name)}</h3><div class="meta">${esc(c.designation)} · ${esc(recordCollegeName(c))}</div></div><span class="pill">${esc(c.preferred)}</span></div><div class="contact-box"><div class="title">${esc(c.phone||"Phone not added")}</div><div class="meta">${esc(c.email||"Email not added")}</div></div><div class="button-row" style="margin-top:10px"><button class="btn ghost editC" data-id="${c.id}">Edit</button><button class="btn danger deleteC" data-id="${c.id}">Delete</button></div></article>`).join(""):'<div class="empty">No contacts added yet.</div>';document.querySelectorAll(".editC").forEach(b=>b.onclick=()=>openForm("contact",b.dataset.id));document.querySelectorAll(".deleteC").forEach(b=>b.onclick=async()=>{if(confirm("Delete this contact?")){const {error}=await sb.from("contacts").delete().eq("id",b.dataset.id);if(error)return alert(error.message);await loadAll()}})}
